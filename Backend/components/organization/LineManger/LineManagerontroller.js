...
module.exports.assignedDataTolineManager = async (req, res, next) => {
  ...
  try {
    let fetchDuplicateData = await lineManagerDAL.findDuplicate(
      empId,
      linemgrId
    );
    if (!fetchDuplicateData.length > 0) {
      ...
      let { firstName, lastName } = await lineManagerDAL.findassignUser(
        linemgrId
      );
      ...
      return res.json({
        status: 200,
        message: `Successfully assigned to ${firstName + " " + lastName}`,
        data: result,
      });
    } else {
  ...
  try {
    ...
    if (result.assignedId != null) {
      ...
      return res.status(200).json({
        status: 200,
        data: "success",
        message: `${employeeName} already assigned to ${
          firstName + " " + lastName
        }`,
        flag: true,
      });
